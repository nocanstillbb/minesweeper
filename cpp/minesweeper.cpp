...
MinesweeperModule::~MinesweeperModule()
{
...
jsi::Object MinesweeperModule::getMinesVm(jsi::Runtime &rt)
{
...
// 3x3 卷积函数
Eigen::MatrixXf MinesweeperModule::conv3x3(const Eigen::MatrixXf &input, const Eigen::MatrixXf &kernel)
{
    int kSize = 3; // 卷积核大小
    int pad = 1;   // 填充大小
...
void MinesweeperModule::private_initCells(jsi::Runtime *rt)
{
...
void MinesweeperModule::open(jsi::Runtime &rt, int index)
{
    private_open(index, &rt);
}

void MinesweeperModule::private_open(int index, jsi::Runtime *rt)
{
...
void MinesweeperModule::initCells(jsi::Runtime &rt)
{
    private_initCells(&rt);
}

void MinesweeperModule::regen(jsi::Runtime &rt)
{
    private_regen();
    this->minesVm->instance()->mode = 0;  // 重置为非旗模式
    this->minesVm->notifyUi(&rt, "mode"); // 通知UI更新模式
}
void MinesweeperModule::private_regen()
{
...
void MinesweeperModule::recurse_open(int i)
{
    if (this->minesVm->instance()->status >= 2) // 如果不是 playing 状态,则不处理
        return;
    int rows = this->minesVm->instance()->row_num;
...
void MinesweeperModule::refreshETime()
{
    while (!exitFlag)
    {
