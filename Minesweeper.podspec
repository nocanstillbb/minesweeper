...
Pod::Spec.new do |s|
  ...
  s.source       = { :git => "https://github.com/nocanstillbb/minesweeper.git", :tag => "#{s.version}" }
  ...
  s.script_phase = {
    :name => "Sync Minesweeper Codegen",
    #:execution_position => :after_compile,
    :execution_position => :before_compile,
    :shell_path => "/bin/sh",
    :script => <<-SCRIPT
      echo "🔹 Syncing Minesweeper codegen files.." >&2
      ...
      mkdir -p "$TARGET_DIR/RNMinesweeperSpec"
      #rsync -av  --include='*/' --include='*.hpp'  --include='*.h' --exclude='*' "$GENERATED_DIR/RNMinesweeperSpec/" "$TARGET_DIR/RNMinesweeperSpec/"
